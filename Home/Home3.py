...
import random
d = int(input('(Задача №1)\nСколько списков желаете задать (указать числом):\n'))
n = 15
for a in range (0, d):
       random.seed(a)
       m = [random.randint(1,100) for i in range(n)]
       import math
       c = []
       for i in m:
           x = math.sqrt(i)
       ...
       print (f'Список (m) = {m}')
       if not c:
           print('Список (c) пуст, поскольку элементы исходного сгенерированного списка (m) не имеют цлочисленных корней')
       else:
           print(f'Список (c) = {c}')